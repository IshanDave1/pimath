...
class Function:
    ...
    def __mul__(self, other):
        if isinstance(other, Function):
            val_at = lambda x: self.val_at(x) * other.val_at(x)
            deri_at = lambda x: self.deri_at(x) * other.val_at(x) + self.val_at(x) * other.deri_at(x)
            strp = f"({self.strp} * {other.strp})"
            return Function(val_at, deri_at, strp)
        elif isinstance(other, (int, float)):
            val_at = lambda x: self.val_at(x) * other
    ...
    def composition(self, other):
        val_at = lambda x: self.val_at(other.val_at(x))
        deri_at = lambda x: self.deri_at(other.val_at(x)) * other.deri_at(x)
        strp = f"{self.strp.replace('x' , other.strp)}"
        return Function(val_at, deri_at, strp)


def cf(const):  # sourcery skip: instance-method-first-arg-name
...
def sinf(func):
    return Function(lambda x: math.sin(func.val_at(x)), lambda x: math.cos(func.val_at(x)) * func.deri_at(x),
                    f"sin({func.strp})")


def cosf(func):
    return Function(lambda x: math.cos(func.val_at(x)), lambda x: -math.sin(func.val_at(x)) * func.deri_at(x),
                    f"cos({func.strp})")
